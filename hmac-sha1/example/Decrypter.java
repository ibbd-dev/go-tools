...
public class Decrypter {
    ...
    public static byte[] decryptSafeWebStringToByte(String webString,
        ...
        SecretKey encryptionKey = new SecretKeySpec(secretKey.getBytes(), "HmacSHA1");
        SecretKey integrityKey = new SecretKeySpec(signKey.getBytes(), "HmacSHA1");
        byte[] retStr = null;
        try {
            retStr = decryptSafeWebString(webString, encryptionKey, integrityKey);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return retStr;
